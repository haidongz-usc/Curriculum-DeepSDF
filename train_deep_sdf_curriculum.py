...
def main_function(experiment_directory, continue_from, batch_split, args):
    ...
    arch = __import__("networks." + specs["NetworkArchTrain"], fromlist=["Decoder"])
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for sdf_data, indices in sdf_loader:
            ...
            for subbatch in range(batch_split):
                ...
                pred_sdf = decoder(inputs, epoch=epoch)

                if enforce_minmax:
                    pred_sdf = deep_sdf.utils.threshold_min_max(
                        pred_sdf, min_vec, max_vec
                    )
                loss = loss_l1(pred_sdf, sdf_gt, eps)
                loss *= (1+ lamb * torch.sign(sdf_gt) * torch.sign(sdf_gt-pred_sdf))
        ...
        logging.info("epoch {a} eps {b} loss {c:1.9f} ...".format(a=epoch, b=eps, c=float(sum(eloss))/len(eloss) ))
...
if __name__ == "__main__":
    ...
    main_function(args.experiment_directory, args.continue_from, int(args.batch_split), args)
